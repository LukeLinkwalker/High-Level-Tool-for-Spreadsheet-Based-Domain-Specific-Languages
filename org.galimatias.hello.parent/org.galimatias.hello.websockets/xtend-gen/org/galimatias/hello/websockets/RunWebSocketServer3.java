package org.galimatias.hello.websockets;
...
import org.galimatias.hello.websockets.ExpressionWebSocketServer;
...
public class RunWebSocketServer3 {
  public static void main(final String[] args) {
    ...
    ExpressionWebSocketServer server = new ExpressionWebSocketServer(_inetSocketAddress) {
      @Override
      public void run() {
        super.run();
      }
    ...
    Launcher<LanguageClient> launcher = new LSPLauncher.Builder<LanguageClient>().setLocalService(languageServer).setRemoteInterface(LanguageClient.class).setInput(server.in).setOutput(server.out).setExecutorService(
